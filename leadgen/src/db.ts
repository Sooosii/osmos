...
export function upsertLead(db: DatabaseSync, lead: Partial<Lead> & { domain: string }): number {
  ...
  db.prepare(`
    INSERT INTO leads (domain, shop_name, platform, email, instagram, country,
                       ...
                       marka_ortusmesi, urun_ortusmesi, score,
    ...
    ON CONFLICT(domain) DO UPDATE SET
      ...
      urun_ortusmesi      = COALESCE(excluded.urun_ortusmesi, leads.urun_ortusmesi),
  ...
  `).run(
    ...
    sqlDeger(lead.marka_ortusmesi), sqlDeger(lead.urun_ortusmesi), sqlDeger(lead.score ?? 0),
